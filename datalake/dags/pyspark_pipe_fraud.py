...
class BlockFraudPipe(BasePipeline):
    ...
    def data_collect(self, **context):
        ...
        context['ti'].xcom_push(key='df_collect', value=df_raw)


    def api_request(self, **context):
        data = context['ti'].xcom_pull(key='df_collect')
        try:
...
if __name__ == '__main__':
    ...
    pipeline.api_request(d, logger)
    logger.info(f'[OK] Job ( Api Request ) Success!')

    pipeline.store_dataset_fraud(logger)
